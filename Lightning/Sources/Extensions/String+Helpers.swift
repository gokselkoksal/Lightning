...
public extension String {
  ...
  func zap_index(_ distance: Int) -> String.Index {
    return index(startIndex, offsetBy: distance)
  }
  
  func zap_range(from range: NSRange) -> Range<String.Index> {
  ...
  func zap_rangeIntersection(with range: NSRange) -> Range<String.Index>? {
  ...
  func zap_character(at distance: Int) -> Character {
    return self[zap_index(distance)]
  }
  
  func zap_substring(with range: NSRange) -> String {
